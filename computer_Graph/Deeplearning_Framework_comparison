...
常用的深度学习框架，比如Tensorflow，PyTorch 作为一个深度学习平台，在深度学习任务中比 NumPy 这个科学计算库强在哪里呢？
...
不论是动态图还是静态图，它们都属于计算图。计算图是用来描述运算的有向无环图。计算图通常包含两种元素：
...
针对requires_grad属性，自己定义的叶子节点默认为False，而非叶子节点默认为True，神经网络中的权重默认为True。

这里 Function 指的是在计算图中某个节点（node）所进行的运算，比如加减乘除卷积等等之类的，Function 内部有 forward() 和 backward() 两个方法，分别应用于正向、反向传播。
...
从上面这个例子可以看出来，我们在定义b这个指数运算的的Function节点时，Pytorch框架自动为tensor b生成了反向传播所需的求导操作<ExpBackward> 。
...
这个例子比较简单，涉及的最复杂的操作是求平均，但是如果我们把其中的加法和乘法操作换成卷积，那么其实和神经网络类似。我们可以简单地画一下它的正向计算图：

![img](https://img-blog.csdnimg.cn/img_convert/ecdfa7f76db6fad930d95b7bedfe9f50.png)



图1：正向计算图

下面给出了对应的代码，我们定义了input，w1，w2，w3 这三个变量，其中 input 不需要求导结果。对于 l1 来说，因为有一个输入需要求导（也就是 w1 需要），所以它自己默认也需要求导，即 requires_grad=True（如果对这个规则不熟悉，欢迎参考 博文的第一部分 或者直接查看 官方 Tutorial 相关部分）。在整张计算图中，只有 input 一个变量是 requires_grad=False 的。正向传播过程的具体代码如下：
...
![img](https://img-blog.csdnimg.cn/img_convert/32f662edf6a3711711c9f67f569c7122.png)
...
其他的导数计算基本上都类似，因为过程太多，这里就不全写出来了，如果有兴趣的话大家不妨自己继续算一下。接下来我们继续运行代码，并检查一下结果和自己算的是否一致：
...
叶子节点是用户创建的张量，如上图的w1,w2和w3，在Pytorch的实现中，为了节省内存，在梯度反向传播结束后，非叶子节点的梯度都会被释放掉。我们可以用 tensor.is_leaf 来判断它是否是叶子张量（leaf tensor）。
...
答案是：情景1不是 inplace，类似 Python 中的 i=i+1, 而情景2是 inplace 操作
...
动态图
动态图意味着计算图的构建和计算同时发生（define by run）。这种机制由于能够实时得到中间结果的值，使得调试更加容易，动态图每次当我们搭建完一个计算图，然后在反向传播结束之后，整个计算图就在内存中被释放了。如果想再次使用的话，必须从头再搭一遍，参见下边这个例子。

，同时我们将大脑中的想法转化为代码方案也变得更加容易，对于编程实现来说更友好。Pytorch使用的就是动态图机制，因此它更易上手，风格更加pythonic，大受科研人员的喜爱。
...
PyTorch 还有一个特性，叫 eager execution。计算图的构建和计算同时发生，意思就是当遇到 tensor 计算的时候，马上就回去执行计算，也就是，实际上 PyTorch 根本不会去构建正向计算图，而是遇到操作就执行。真正意义上的正向计算图是把所有的操作都添加完，构建好了之后，再运行神经网络的正向传播。
...
动态图 vs 静态图
通过一个例子来对比下动态图和静态图机制在编程实现上的差异，分别基于Pytorch和Tensorflow实现，先来看看Pytorch的动态图
...
————————————————
版权声明：本文为CSDN博主「程序之巅」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/zhuguiqin1/article/details/108775731
